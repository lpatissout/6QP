...
// CORRECTION : Ajouter escapeHtml pour éviter les injections XSS
...
const renderDebugPanel = () => {
    ...
    <div class="debug-panel">
        <div class="flex justify-between items-center mb-2 pb-2 border-b border-gray-700">
            <strong style="color: white;">🔍 Debug Console</strong>
        ...
        <div class="space-y-1">
            ${state.debugLogs.length === 0 
                ...
                : state.debugLogs.slice(-50).reverse().map(log => `
                    <div class="text-xs">
                        <span style="color: #888;">[${log.time}]</span>
                        <span style="color: #0f0;"> ${escapeHtml(log.msg)}</span>
                        ${log.data ? `<div style="color: #0cf; margin-left: 10px;">${escapeHtml(log.data)}</div>` : ''}
                    </div>
                `).join('')
            }
        </div>
...
const renderCard = (num, selected = false, clickable = true, small = false) => {
    ...
    const cursor = clickable ? 'cursor-pointer card-hover' : 'opacity-75';
    const border = selected ? 'ring-4 ring-blue-600 scale-105' : '';
    
    return `
        <div 
            ${clickable ? `onclick="handleCardClick(${num})"` : ''} 
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-2xl p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <button 
                    onclick="createGame()" 
                    class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-6 rounded-lg transition"
                >
                    🎮 Créer une partie
                </button>
                <button 
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-6">
            ...
            <div class="flex gap-3">
                <button 
                    onclick="toggleReady()" 
                    class="flex-1 ${me.ready ? 'bg-gray-300 hover:bg-gray-400' : 'bg-green-500 hover:bg-green-600 text-white'} font-bold py-3 rounded-lg transition"
                >
                ...
                ${isHost ? `
                    <button 
                        onclick="startGame()" 
                        class="flex-1 ${canStart ? 'bg-orange-500 hover:bg-orange-600 text-white' : 'bg-gray-300 text-gray-600 cursor-not-allowed'} font-bold py-3 rounded-lg transition" 
                        ${!canStart ? 'disabled' : ''}
                    >
                        🚀 Lancer la partie
                    </button>
...
const renderGame = () => {
    ...
    // CORRECTION : Gérer l'état finished
    if (state.game.status === 'finished') {
        ...
        <div class="container mx-auto px-4 py-8">
            <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-2xl p-8 text-center">
                ...
                <button 
                    onclick="leaveGame()" 
                    class="bg-orange-500 hover:bg-orange-600 text-white font-bold px-6 py-3 rounded-lg transition"
                >
                    Retour à l'accueil
                </button>
            </div>
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-lg p-4 mb-4 flex justify-between items-center">
                ...
                <div class="flex gap-2 items-center">
                    ${!state.isMobile ? `
                        <button 
                            onclick="toggleDebug()" 
                            class="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded-lg transition text-sm"
                        >
                ...
                <div class="grid grid-cols-2 md:grid-cols-${Math.min(state.game.players.length, 4)} gap-3">
                    ${state.game.players.map(p => `
                        <div class="text-center p-3 rounded-lg ${p.id === state.playerId ? 'bg-orange-100 ring-2 ring-orange-500' : 'bg-gray-100'}">
                            ...
                            ${hasPlayed(p) 
                                ? '<div class="text-green-500 text-sm font-semibold">✓ Joué</div>' 
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-4">
                ...
                ${state.game.waitingForRowChoice && state.game.waitingForRowChoice === state.playerId 
                    ? `<div class="bg-orange-100 border-2 border-orange-500 rounded-lg p-4 mb-4 text-center font-bold text-orange-700">
                        ⚠️ Votre carte (${state.game.pendingCard}) est trop petite ! Choisissez une rangée à ramasser :
                       </div>` 
                    : state.game.waitingForRowChoice 
                    ? `<div class="bg-blue-100 border-2 border-blue-500 rounded-lg p-4 mb-4 text-center font-bold text-blue-700">
                        ⏳ ${escapeHtml(state.game.players.find(p => p.id === state.game.waitingForRowChoice)?.name || 'Un joueur')} doit choisir une rangée...
                       </div>`
                    : ''
                }
                ...
                <div class="space-y-3">
                    ${state.game.rows.map((row, i) => {
                        ...
                        return `
                        <div 
                            class="flex items-center gap-3 p-2 rounded-lg ${canClick ? 'cursor-pointer hover:bg-orange-50 border-2 border-transparent hover:border-orange-500 transition' : 'border border-gray-200'}" 
                            ${canClick ? `onclick="chooseRow(${i})"` : ''}
                        >
                            ...
                            <span class="text-sm font-semibold text-gray-700 bg-gray-100 px-3 py-2 rounded-lg whitespace-nowrap">
                                ${row.length}/5 | ${totalHeads} 🐮
                            </span>
                        </div>
                        `;
                    }).join('')}
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                ${hasPlayed(me) 
                    ...
                    <div class="text-center py-8">
                        <p class="text-2xl mb-2">✓ Vous avez joué votre carte !</p>
                        <p class="text-gray-600 mb-2">En attente des autres joueurs...</p>
                        ${waitingPlayers.length 
                            ? `<p class="text-sm text-gray-500">Attente de : ${escapeHtml(waitingPlayers.join(', '))}</p>` 
                            : ''
                        }
                    </div>
                    ` 
                    ...
                    <p class="text-center text-sm text-gray-600">
                        ${state.selectedCard 
                            ? '👆 Cliquez à nouveau sur la carte pour confirmer' 
...
// CORRECTION : Fonction render principale
const render = () => {
